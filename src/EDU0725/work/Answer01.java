...
public class Answer01 {
     ...
     * @return "짝수" or "홀수"
     */
    public static String utilNumType(int num) {
        String returnData = "결과없음";
        if(num % 2 == 0) {
            returnData = "짝수";
        } else {
            returnData = "홀수";
    ...
    public static int[] utilRandomArray(int min, int max, int leng) {
        int[] returnData = new int[leng];   // 배열 생성
        for (int i = 0; i < leng; i++) {
            /** 
             * Math.random()                : 0 ~ 1 사이의 랜덤 값 (실수값 : 0 ~ 0.9999...)
             * (int) Math.random()          : 정수 변환 
             *   -> (int) Math.random() * 10 이면 0 ~ 9 사이의 정수
             * ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
             * 계산 원리
    ...
    public void example01() {
        ...
        int num1 = 5;
        int num2 = 10;
        System.out.println("num1 = " + num1 + " " + utilNumType(num1));
        System.out.println("num2 = " + num2 + " " + utilNumType(num2));
    }
    ...
    public void example04() {
        ...
        int min = 1;
        int max = 50;
        // System.out.println("랜덤 배열 1 : " + Arrays.toString(utilRandomArray(min, max, 6)));
        // System.out.println("랜덤 배열 2 : " + Arrays.toString(utilRandomArray(min, max, 6)));
        // System.out.println("랜덤 배열 3 : " + Arrays.toString(utilRandomArray(min, max, 6)));
    }
}
